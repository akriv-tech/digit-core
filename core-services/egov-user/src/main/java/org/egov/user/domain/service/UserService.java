...
public class UserService {
	...
	public UserService(UserRepository userRepository, OtpRepository otpRepository, FileStoreRepository fileRepository,
			UserUtils userUtils, PasswordEncoder passwordEncoder, EncryptionDecryptionUtil encryptionDecryptionUtil,
			TokenStore tokenStore, @Value("${default.password.expiry.in.days}") int defaultPasswordExpiryInDays,
	...
	public User getUniqueUser(String userName, String tenantId, UserType userType) {
		...
		UserSearchCriteria userSearchCriteria = UserSearchCriteria.builder().userName(userName)
				.tenantId(userUtils.getStateLevelTenantForCitizen(tenantId, userType)).type(userType).build();
	...
	public Boolean validateOtp(User user) {
		Otp otp = Otp.builder().otp(user.getOtpReference())
				.identity(user.getType().equals(UserType.EMPLOYEE) ? user.getUsername() : user.getMobileNumber())
