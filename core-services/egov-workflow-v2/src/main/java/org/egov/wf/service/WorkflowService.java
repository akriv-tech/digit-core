...
public class WorkflowService {
    ...
    public List<ProcessInstance> search(RequestInfo requestInfo,ProcessInstanceSearchCriteria criteria){
        ...
        if(requestInfo == null || requestInfo.getUserInfo() == null)
            throw new CustomException("UNAUTHORIZED_ACCESS", "User information is required to search processes. Access Denied (403)");
...
//        }
    ...
    /**
     * Validates if current user has access to the process instance based on their role and association
    ...
    public void validateUserAccessToProcesses(RequestInfo requestInfo, List<ProcessInstance> processInstances) {
        if(CollectionUtils.isEmpty(processInstances)) {
            return; // Empty list, no validation needed
        }

        User currentUser = requestInfo.getUserInfo();
        if(currentUser == null) {
            throw new CustomException("UNAUTHORIZED_ACCESS", "User information is required to access processes. Access Denied (403)");
        }
        ...
        boolean isEmployee = hasRole(userRoles, "EMPLOYEE");
        // Check if user has CITIZEN role
        boolean isCitizen = hasRole(userRoles, "CITIZEN");
        // Check if user has architect/creator role (BPA_ARCHITECT, etc.)
        boolean isArchitect = isArchitectRole(userRoles);

        for(ProcessInstance processInstance : processInstances) {
            boolean hasAccess = false;

            // EMPLOYEE: Check if current user is in assignees list
            if(isEmployee) {
                hasAccess = isUserInAssignees(processInstance, currentUserUuid);
            }
            // ARCHITECT/CREATOR (e.g., BPA_ARCHITECT): Check if current user is the assigner
            else if(isArchitect) {
                hasAccess = isUserAssigner(processInstance, currentUserUuid);
            }
            // CITIZEN: Check if current user is the applicant
            else if(isCitizen) {
                // For CITIZEN, check if user is the creator/assigner (submitter of the application)
                hasAccess = isUserAssigner(processInstance, currentUserUuid);
            }

            if(!hasAccess && !isEmployee && !isArchitect && !isCitizen) {
                // No matching role found
                throw new CustomException("UNAUTHORIZED_ACCESS", 
                    "User with role does not have access to this process. Access Denied (403)");
            }
        }
    }
    ...
    /**
     * Checks if role is an architect/creator type (BPA_ARCHITECT, CITIZEN, etc.)
     * @param roles List of user roles
     * @return true if user has architect role
     */
    private boolean isArchitectRole(List<Role> roles) {
        if(CollectionUtils.isEmpty(roles)) {
            return false;
        }
        return roles.stream()
    ...
    /**
     * Checks if current user UUID is the process instance assigner (creator/submitter)
     * @param processInstance The process instance to check
     * @param userUuid The user UUID to look for
    ...
    private boolean isUserAssigner(ProcessInstance processInstance, String userUuid) {
        ...
        // Check if user is in assigners (alternative field name)
        if(!CollectionUtils.isEmpty(processInstance.getAssigners())) {
            boolean isAssigner = processInstance.getAssigners().stream()
                .anyMatch(user -> userUuid.equalsIgnoreCase(user.getUuid()));
